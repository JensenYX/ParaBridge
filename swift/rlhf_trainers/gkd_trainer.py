...
class GKDTrainer(RolloutTrainerMixin, SwiftMixin, HFGKDTrainer):
    ...
    def __init__(self, model: Optional[Union[PreTrainedModel, nn.Module, str]] = None, *_args, **kwargs):
        ...
        gkd_rollout_batch_size = kwargs.pop('gkd_rollout_batch_size', None)
        ...
        # Rollout sync interval: only sync weights to server every N steps
        self.gkd_rollout_batch_size = gkd_rollout_batch_size
        # Start at 1 (as if we just synced): vLLM server already has the initial weights,
        # no need to do an expensive 60GB sync before any parameters have been updated.
        self._rollout_sync_counter = 1
        ...
        # Collect media pad token IDs that vLLM may generate but should not appear in text responses.
        # These cause masked_scatter failures in _post_encode when the mask count exceeds audio_embeds size.
        self._media_pad_token_ids = set()
        try:
            thinker_config = getattr(self.model.config, 'thinker_config', None) or self.model.config
            for attr in ('audio_token_id', 'audio_token_index', 'image_token_id', 'video_token_id'):
                tid = getattr(thinker_config, attr, None)
                if tid is not None:
                    self._media_pad_token_ids.add(tid)
        except Exception:
            pass

    def _sanitize_response_token_ids(self, generated_inputs):
        """Remove media pad tokens from vLLM-generated response_token_ids.

        vLLM may generate special media pad tokens (audio/image/video) in the response,
        which cause masked_scatter failures during _post_encode because the mask count
        exceeds the actual media embedding size.
        """
        if not self._media_pad_token_ids:
            return
        for data in generated_inputs:
            if 'response_token_ids' not in data:
                continue
            token_ids = data['response_token_ids']
            if isinstance(token_ids, list) and token_ids:
                if isinstance(token_ids[0], list):
                    # Multi-turn: List[List[int]]
                    data['response_token_ids'] = [
                        [t for t in turn if t not in self._media_pad_token_ids]
                        for turn in token_ids
                    ]
                else:
                    # Single-turn: List[int]
                    data['response_token_ids'] = [
                        t for t in token_ids if t not in self._media_pad_token_ids
                    ]
    ...
    def _build_opsd_teacher_data(self, inputs):
        ...
        or by inserting a system message via teacher_system field.

        Returns None if neither teacher_prompt nor teacher_system is available in all examples.
        """
        has_teacher_prompt = all('teacher_prompt' in data and data['teacher_prompt'] for data in inputs)
        has_teacher_system = all('teacher_system' in data and data['teacher_system'] for data in inputs)
        if not has_teacher_prompt and not has_teacher_system:
            return None
        ...
        teacher_data = []
        for data in inputs:
            teacher_item = {k: v for k, v in data.items() if k not in ('teacher_prompt', 'teacher_system')}
            messages = [dict(m) for m in data.get('messages', [])]
            if messages and messages[-1]['role'] == 'assistant':
                messages.pop()
            if has_teacher_prompt:
                for msg in reversed(messages):
                    if msg['role'] == 'user':
                        msg['content'] = data['teacher_prompt']
                        break
            if has_teacher_system:
                # Insert system message at the beginning if not already present
                if not messages or messages[0]['role'] != 'system':
                    messages.insert(0, {'role': 'system', 'content': data['teacher_system']})
                else:
                    messages[0]['content'] = data['teacher_system']
            teacher_item['messages'] = messages
            teacher_data.append(teacher_item)
        return teacher_data

    def _batch_rollout_and_cache(self, current_inputs, current_teacher_data):
        """Batch rollout: sync weights once, rollout current batch via vLLM, cache the result.

        Instead of prefetching from dataloader (which conflicts with HF trainer's own iterator),
        we cache rollout results one at a time. The cache stores (generated_inputs, teacher_data)
        and the `_rollout_sync_counter` tracks how many steps since last weight sync.
        When counter reaches gkd_rollout_batch_size, we sync weights again.
        """
        # Sync weights to server (the expensive 60GB transfer)
        self._last_loaded_step = -1
        logger.info(f'Batch rollout: syncing weights to server...')

        # Rollout current inputs via vLLM
        processed = self._preprocess_inputs(current_inputs)
        generated = self._fast_infer(processed)
        self._sanitize_response_token_ids(generated)
        cached_td = None
        if current_teacher_data is not None:
            for i, gen_data in enumerate(generated):
                current_teacher_data[i]['messages'].append(dict(gen_data['messages'][-1]))
                if 'response_token_ids' in gen_data:
                    current_teacher_data[i]['response_token_ids'] = gen_data['response_token_ids']
                current_teacher_data[i]['add_eos'] = False
            cached_td = current_teacher_data

        # Reset sync counter
        self._rollout_sync_counter = 1
        logger.info(f'Batch rollout: synced and generated. Next sync in {self.gkd_rollout_batch_size - 1} steps.')

        return generated, cached_td

    def _rollout_no_sync(self, inputs, teacher_data):
        """Rollout without syncing weights to server. Uses stale (but recent) weights."""
        processed = self._preprocess_inputs(inputs)

        # Prevent _fast_infer from syncing by pretending we just synced
    ...
    def _compute_jsd_loss(self, student_logits, teacher_output: TeacherOutput, labels):
        ...
        if opsd_teacher_labels is not None:
            ...
            # Per-sample alignment: ensure each sample has the same number of
            # response tokens for student and teacher.  When teacher has a longer
            # prompt the response may be truncated differently; trim trailing
            # response tokens of the longer side *per sample* to keep position-
            # wise JSD meaningful.
            batch_size = student_mask.shape[0]
            for b in range(batch_size):
                s_cnt = student_mask[b].sum().item()
                t_cnt = teacher_mask[b].sum().item()
                if s_cnt != t_cnt:
                    min_cnt = min(s_cnt, t_cnt)
                    if s_cnt > min_cnt:
                        indices = student_mask[b].nonzero(as_tuple=False).squeeze(-1)
                        student_mask[b, indices[min_cnt:]] = False
                    if t_cnt > min_cnt:
                        indices = teacher_mask[b].nonzero(as_tuple=False).squeeze(-1)
                        teacher_mask[b, indices[min_cnt:]] = False
    ...
    def training_step(self,
        ...
        with profiling_context(self, 'get_completions'):
            if self._get_random_num() <= self.lmbda:
                ...
                if args.use_vllm:
                    # Batched sync mode: sync weights every gkd_rollout_batch_size steps.
                    # Between syncs, rollout uses stale (but recent) student weights.
                    if self.gkd_rollout_batch_size and self.gkd_rollout_batch_size > 1:
                        need_sync = (not hasattr(self, '_rollout_sync_counter')
                                     or self._rollout_sync_counter >= self.gkd_rollout_batch_size)
                        if need_sync:
                            generated_inputs, teacher_data = self._batch_rollout_and_cache(
                                inputs, teacher_data)
                        else:
                            generated_inputs, teacher_data = self._rollout_no_sync(
                                inputs, teacher_data)
                        with self._template_context(self.template):
                            encoded_inputs = self._prepare_batch_inputs(
                                generated_inputs, encode_prompt_only=False)
                        if teacher_data is not None:
                            with self._template_context(self.template):
                                encoded_inputs['_opsd_teacher_inputs'] = self._prepare_batch_inputs(
                                    teacher_data, encode_prompt_only=False)
                    else:
                        # Original per-step rollout
                        processed_inputs = self._preprocess_inputs(inputs)
                        generated_inputs = self._fast_infer(processed_inputs)
                        self._sanitize_response_token_ids(generated_inputs)
