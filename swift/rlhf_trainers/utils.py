...
def patch_stateless_process_group_for_ipv6():
    ...
    def _patched_stateless_pg_create(
        ...
        """Patched version of StatelessProcessGroup.create that supports both IPv4 and IPv6.

        This patch fixes the "Address already in use" error in multi-GPU training by:
        1. Only allowing rank 0 to bind to the socket
        2. Setting SO_REUSEADDR socket option for both IPv4 and IPv6
        3. Having other ranks connect as clients via TCPStore
        """
        # Determine socket family based on address type
        launch_server = rank == 0
        is_ipv6 = is_valid_ipv6_address(host)
        socket_family = socket.AF_INET6 if is_ipv6 else socket.AF_INET

        if launch_server:
            listen_socket = socket.socket(socket_family, socket.SOCK_STREAM)
            # Critical fix: Set SO_REUSEADDR for both IPv4 and IPv6
            # This prevents "Address already in use" errors in multi-GPU training
