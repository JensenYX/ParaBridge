...
class VllmEngine(InferEngine):
    ...
    def _prepare_generation_config(self, request_config: RequestConfig) -> SamplingParams:
        ...
        # Pass prompt_logprobs to vLLM if requested (used by teacher_model_server in GKD)
    ...
    def _create_chat_completion_response(
        ...
        # Include prompt_logprobs if requested (for teacher_model_server in GKD)
        prompt_logprobs_data = None
        if request_config.prompt_logprobs is not None and hasattr(result, 'prompt_logprobs') and result.prompt_logprobs:
            prompt_logprobs_data = []
            for plp in result.prompt_logprobs:
                if plp is None:
                    prompt_logprobs_data.append(None)
                else:
                    prompt_logprobs_data.append({
                        str(token_id): {'logprob': lp.logprob, 'rank': lp.rank}
                        for token_id, lp in plp.items()
                    })
        return ChatCompletionResponse(
            ...
            prompt_logprobs=prompt_logprobs_data,
